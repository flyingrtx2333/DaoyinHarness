...
2026-09-06 当前生产使用 PostgreSQL 与 WebSocket，运行时和静态版本为 `efe56eb`；332 项 Windows 测试、逐行迁移校验及真实线上连续两轮对话通过。数据库位置、备份、发布版本与验收边界见 [最新发布记录](../../deployment/RELEASE-20260906-WS-POSTGRES.md)。下文早期版本为历史记录。
...
打开 SQLite 数据库本身不会接管 running 任务。旧 SQLite 试运行入口先取得单实例租约，再把旧任务追加为 interrupted；不自动重放模型或外部操作。租约有效时拒绝第二个执行实例，旧实例丢失租约后不能追加结果。首次从无租约旧版本升级必须先停止旧服务并确认退出；不能让不认识租约表的旧二进制继续运行。当前标准入口已改为上述 PostgreSQL 实现，仍不提供多 Worker 调度或跨机器故障接管。`recoverInterruptedRuns()` 保留为受控管理操作，租约模式下调用者也必须具有执行所有权。

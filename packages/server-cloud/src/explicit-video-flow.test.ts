...
describe("explicit video flow", () => {
  ...
  it("starts with capability discovery without calling the paid generation tool", async () => {
    ...
    const model = createExplicitVideoFlow(run)!;
    const reply = await model.complete({ messages: [{ role: "user", content: run.userMessage }],
      tools: ["story_video_options", "story_estimate_video", "request_video_confirmation", "story_create_video"].map(name => ({
        name, description: name, inputSchema: {}, category: "extension" as const, mutating: name === "story_create_video",
      })), systemPrompt: { stableText: "", dynamicText: "", sections: [] }, signal: new AbortController().signal });

...
server.listen(SOCKET, () => { const gid = Number(process.env.HARNESS_RESOURCE_GID); if (Number.isSafeInteger(gid) && gid > 0) void chown(SOCKET, 0, gid).then(() => chmod(SOCKET, 0o660)); });

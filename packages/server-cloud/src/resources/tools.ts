...
export const RESOURCE_DEFINITIONS: Readonly<Record<ResourceToolName, { description: string; mutating: boolean; inputSchema: Record<string, unknown> }>> = {
  ...
  workspace_create: { description: "Create a language- and task-neutral cloud workspace from an empty tree, Git source, upload or snapshot. Omit runtime to use the operator-signed default image.", mutating: true, inputSchema: object({ title: { type: "string", minLength: 1, maxLength: 120 }, source: { type: "object" }, runtime: { type: "object" } }, ["title"]) },

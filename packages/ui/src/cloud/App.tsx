...
export function App(): React.JSX.Element {
  ...
  return <div className="workbench">
    ...
    <aside className={`sidebar ${sidebar ? "is-open" : ""}`} aria-label="会话导航">
      ...
      <nav className="workspace-tabs" aria-label="工作台导航"><button aria-current={view === "chat" ? "page" : undefined} onClick={() => { setView("chat"); setSidebar(false); }}>会话</button><button aria-current={view === "plugins" ? "page" : undefined} onClick={browsePlugins}>插件</button></nav>
    ...
    <main id="conversation" className="main" tabIndex={-1}>
      <header className="topbar">
        ...
        <div className="session-heading"><h1>{view === "plugins" ? "插件" : session?.title || "新会话"}</h1><span>{view === "plugins" ? "业务能力与接入状态" : `${plugin?.name ?? "插件未识别"} · 只读工具`}</span></div>
      ...
      <div className="composer-area" hidden={view !== "chat"}>
        ...
        <form className="composer" onSubmit={(event) => { event.preventDefault(); void send(); }}>
          ...
          <div className="composer-controls"><div className="composer-plugins"><PluginPicker selectedId={plugin?.id ?? ""} onSelect={(id) => usePlugin(id, false)} onBrowse={browsePlugins} busy={pluginBusy} /><button type="button" className="selected-plugin" onClick={browsePlugins} aria-label={`查看${plugin?.name ?? "会话插件"}详情`}>{plugin?.name ?? "选择插件"}</button>{active && <span className="composer-busy">进行中</span>}</div>{submitting || active ? <button type="button" className="stop-button" disabled={cancelling || active?.cancelRequested} onClick={() => { void cancel(); }}>{cancelling || active?.cancelRequested ? "正在停止…" : "停止生成"}</button> : <button type="submit" className="primary" disabled={!draft.trim() || phase !== "ready" || loading || !!pending || !plugin}>发送 <span aria-hidden="true">↑</span></button>}</div>
        </form>

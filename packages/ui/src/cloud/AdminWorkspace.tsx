...
import "./admin.css";
type Section="overview"|"agent"|"routing";
export function AdminWorkspace({client,workbenchClient,onDenied}:{client:EvaluationClient;workbenchClient:WorkbenchClient;onDenied:()=>void}):React.JSX.Element{
 ...
 return <div className="admin-workspace">
  ...
  <nav className="evaluation-tabs" aria-label="后台页面标签"><button aria-current={section==="overview"?"page":undefined} onClick={()=>setSection("overview")}>测试总览</button><button aria-current={section==="agent"?"page":undefined} onClick={()=>setSection("agent")}>Agent 评估</button><button aria-current={section==="routing"?"page":undefined} onClick={()=>setSection("routing")}>能力路由</button></nav>

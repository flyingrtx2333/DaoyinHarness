...
export function PluginCatalog({ onSelect, busy }: PluginActions): React.JSX.Element {
  ...
  return <section className="plugin-catalog" aria-label="业务插件">
    ...
    <div className="plugin-grid">{matches.map((plugin) => <article className={`plugin-card ${plugin.status}`} key={plugin.id} aria-labelledby={`catalog-${plugin.id}`}>
      ...
      {plugin.status === "available" ? <button className="plugin-use" aria-label={`使用${plugin.name}`} disabled={busy} onClick={() => onSelect(plugin.id)}>使用</button> : <span className="plugin-state">待接入</span>}
    </article>)}</div>

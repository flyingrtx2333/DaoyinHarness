...
describe("workspace tools", () => {
  it("reads, writes and patches files through the confined workspace service", async () => {
    ...
    const read = await registry.execute({ id: "call_read", name: "read_file", input: { path: "notes.txt" } }, new AbortController().signal);
    ...
    const patch = await registry.execute({ id: "call_patch", name: "apply_patch", input: { path: "created.txt", expected: "hello", replacement: "updated" } }, new AbortController().signal);
  ...
  it("does not expose process execution from the workspace pack", async () => {
    ...
    const result = await registry.execute({ id: "call_process", name: "run_package_script", input: { script: "test" } }, new AbortController().signal);
    expect(result).toMatchObject({ ok: false, code: "TOOL_NOT_FOUND" });
  });

  it("returns a stable failure for unknown tools", async () => {
    const registry = new ToolRegistry();
    const result = await registry.execute({ id: "call_unknown", name: "unknown", input: {} }, new AbortController().signal);
    expect(result).toMatchObject({ ok: false, code: "TOOL_NOT_FOUND" });
  });
});

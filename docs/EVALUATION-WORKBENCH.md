...
测试输入是题目列表的唯一来源：每个非空行即时计为一题，修改、删除、清空和调整重复次数都会同步更新计数。不再要求点击「整理用例」；同样的问题重复输入仍计为独立题目。题数乘以重复次数最多五次，超限明确报错，不截断输入。

桌面采用输入主区与右侧历史实验，窄屏上下排列。实验名称、重复次数、单题模型调用上限和可选核对条件收进「运行设置」。点击「开始测试」只打开本批输入与费用确认框，确认后才提交；取消不会创建实验。实际任务、计数和确认框使用同一份冻结配置，不提交旧输入。

总调用预算为题数 × 重复次数 × 单题上限，不再为未启用的模型判分虚增一次；空输入不显示一笔虚假调用。必须同时发布工作台与独立评估服务的新契约。既有 Cookie、CSRF、角色检查、单题执行上限和提交幂等保持不变。

结果区优先展示实际回复、任务完成数、待复核数、耗时与实际模型调用；详细核验、工具、任务编号、版本和未测指标默认折叠，导出报告继续保留完整信息。没有真值的召回率不改成零或通过率。

当前执行方式为真实账号链路，详见 [真实评估发布记录](../deployment/RELEASE-20260907-EVALUATION-LIVE.md) 和 [ADR-0019](adr/0019-live-evaluation-deployment.md)。下文为首版历史说明，50 题、手动整理、程序回放、隔离业务夹具和独立模型判分等描述不适用于当前版本，也不构成执行旧模拟测试的授权。
...
云端工作台「插件」下分为「插件目录」和「测试评估」。也可使用 `https://harness.daoyintech.com/#plugins-evaluation` 定位页面，但 URL 不授予权限。普通账号和企业管理员看不到测试标签；直接访问会显示拒绝，所有后端读写接口同样拒绝。

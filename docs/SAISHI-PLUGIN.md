...
赛事列表和详情由主平台桥接层补充不含存储地址的 `cover_image_id`，工作台通过账号作用域代理显示 `event_cover`；赛事封面继续属于赛事读取权限。`saishi_list_images` 仍优先返回原始图片素材和精彩照片；当默认图库没有照片、Saishi 原本准备降级为视频预览时，如果赛事已有封面，主平台会改为返回明确的 `event_cover`，只有没有封面或用户明确请求视频预览时才显示 `video_preview`。图片复用当前账号权限，无需二次授权；聊天记录不保存存储地址或凭证。更多结果继续使用返回的 `source` 和 `next_after_id`，同轮不重复查询或无限翻页。详见 [ADR-0017](adr/0017-account-scoped-event-images.md)。
...
插件目录显示「赛事只读」已启用。未登录时进入道引账号登录，已登录账号自动接入，不显示插件二次授权，也不通过 URL 或页面重载切换应用。

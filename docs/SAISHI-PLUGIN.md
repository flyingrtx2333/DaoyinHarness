...
2026-09-06：第一阶段源码已接入，未运行测试、构建、实际模型调用或部署。服务可用性仍需部署后验证。
...
保留现有测试源码，没有执行 WSL/Windows 测试、类型检查或构建。待操作者核查：八项真实查询，跨资源拒绝，权限撤销，会话隔离，模型次数限额，断线后原请求恢复，以及原官网知识模式不受影响。不能把代码审查写成通过验收。

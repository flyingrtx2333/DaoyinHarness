...
默认入口是官网公开知识，支持会话列表、搜索、新建、多轮回答、检索状态、公开资料引用、刷新恢复与取消。访客授权有效期为 30 分钟；授权失效后需要明确重新进入，新访客空间不能读取旧空间。赛事入口 `?app=saishi` 已接通道引账号，直接使用当前业务空间的读取权限并按账号隔离会话。文件上传与本机文件/进程工具尚未接入此云端入口。

左侧“插件”展示业务能力与接入状态；输入框左下角“＋”选择插件。官网知识直接进入，赛事通过道引账号登录进入，无需手动凭据；短剧制作、网站与应用、文旅影像列为待接入。具体能力与自动继承权限规则见 [插件盘点](PLUGINS.md)，当前版本见 [上线记录](../deployment/RELEASE-20260906-ACCOUNT-ACCESS.md)。

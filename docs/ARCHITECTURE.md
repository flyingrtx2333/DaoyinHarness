...
The cloud runtime exposes bounded orchestration only to authenticated non-public application identities. `delegate_agent` creates a real child `cloud_run` in the same scoped session, so PostgreSQL `cloud_runs` and append-only `cloud_events` remain the authoritative audit trail instead of process-local orchestration state. Child runs use the same execution identity and authorization policy, inherit the parent cancellation signal, exclude memory mutation and all orchestration tools, and share a hard parent+children model-call budget. Deterministic request IDs fence accidental replay: only an already-completed child may be reused; an incomplete prior child is not automatically re-executed. `delegate_parallel` allows at most three independent child runs and preserves input order when aggregating visible results. `workflow_run_inline` executes dependent child runs sequentially and explicitly passes bounded prior final text to the next child; hidden reasoning is never transferred. Public company Q&A receives none of these orchestration tools.
